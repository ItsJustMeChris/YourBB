...
class AuthModel
{
    ...
    public static function logout() {
        ...
        session_destroy();
        $_SESSION = array();
        die(json_encode(["success" => self::$logoutMessage]));
    }
        ...
        if ($result->num_rows > 0)
        {
            while ($row = mysqli_fetch_array($result))
            {
                if (password_verify($f_password,$row['password']))
                {
                    ...
                    die(json_encode(["success" => self::$loggedInMessage]));
                }
                else
                {
    ...
    public static function register($data)
    {
        ...
        $passwordHash = db::escape(password_hash($data['password'], PASSWORD_BCRYPT, ['cost' => 11]));
